...
export const programID = new PublicKey(
  "HN8JJdo8c9iLQPzbTqjoioW61BDgyevHaGkCPSYLuDy",
);
...
export const createConfigureWebAuthnTx = async (
  ...
  try {
    ...
    if (webauthnPubkey.length === 65 && webauthnPubkey[0] === 0x04) {
      ...
      console.warn(
        `Khóa công khai WebAuthn không đúng định dạng: ${webauthnPubkey.length} bytes`,
      );
      ...
      if (webauthnPubkey.length > 0) {
        ...
        new Uint8Array(compressedKey).set(
          new Uint8Array(
            webauthnPubkey.subarray(0, Math.min(webauthnPubkey.length, 32)),
          ),
          1,
        );
      }
    ...
    console.log(
      "Khóa công khai WebAuthn (nén):",
      compressedKey.toString("hex"),
    );
...
export const createAddGuardianTx = (
  ...
  try {
    ...
    const data = Buffer.concat(
      dataBuffers.map((buffer) => new Uint8Array(buffer)),
    );
